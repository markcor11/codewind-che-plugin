...
# Get the che ingress host
CHE_INGRESS_HOST=$(kubectl get ingress -n che che-ingress -o jsonpath="{.spec.rules[0].host}")
sed -i "s/CHE_INGRESS_HOST_PLACEHOLDER/$CHE_INGRESS_HOST/g" /scripts/kube/codewind_template.yaml

# Get the che ingress domain
CHE_INGRESS_DOMAIN=$(echo $CHE_INGRESS_HOST | sed -e "s/che-che.//g")
sed -i "s/CHE_INGRESS_DOMAIN_PLACEHOLDER/$CHE_INGRESS_DOMAIN/g" /scripts/kube/codewind_template.yaml
